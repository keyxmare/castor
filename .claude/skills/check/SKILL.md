...
- `check-fast` = gates **rapides** sans tests (format + lint + analyse statique + typecheck). C'est la cible appelée par le hook `commit-gate` pour ne pas pénaliser chaque commit. Voir le `Makefile`.
